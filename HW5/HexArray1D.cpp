...
*/
HexArray1D::HexArray1D() : AbstractHex(), hexCells(nullptr){
...
*/
HexArray1D::HexArray1D (const int hexSize, const int opponent) : AbstractHex(hexSize, opponent), hexCells(nullptr){
...
*/
HexArray1D::HexArray1D(const string& fileName) : AbstractHex(0, 0), hexCells(nullptr) {
...
// Copy Constructor
HexArray1D::HexArray1D(const AbstractHex* absHex) : AbstractHex(absHex->getSize(), absHex->getOpponent()) {

    createCells();
    for (int i=0; i < size; ++i) 
        for (int j=0; j < size; ++j) 
            this->setCell(i,j, absHex->operator()(i,j));
}
...
void HexArray1D::play() {
    ...
    hexCells[fm.getRow() * size + fm.getCol()].setVal(X);
...
*/
void HexArray1D::play(int row, int col) {

    if (turn == 0) {                    // Player 1
        hexCells[row*size + col].setVal(O);
        lastmove.setRowColVal(row, col, O);
        userMarked++;
    }
    else {                              // Player 2
        hexCells[row*size + col].setVal(X);
...
*/
void HexArray1D::print(ostream& stream) const { 
    ...
    for (int i = 0; i < size; ++i) {       
        ...
        for (int j = 0; j < size; ++j) 
            switch(hexCells[i*size + j].getVal()){
                case 0:
                    stream << ". ";
                    break;
                case 1:
...
void HexArray1D::createCells() {
    ...
    if(hexCells == nullptr) {
        ...
        for (int i=0; i<size; ++i)
            for (int j=0; j < size; ++j)
                hexCells[i*size + j] = Cell();
        
        for (int i=0; i<size; ++i)
            for (int j=0; j < size; ++j)
                hexCells[i*size + j].setRowColVal(i, j, DOT);
...
FinalMove HexArray1D::ParseMove_AI() {
    ...
    if(posList.getSize() == 0){
        do {
            row = dist(rng);
            col = dist(rng);
        }while (hexCells[row*size + col].getVal() != DOT);
...
void HexArray1D::ParseMove() {
    ...
    do {
        ...
        if (isCommand) {
            ...
            if (turn == 0 && hexCells[(row-1)*size + col].getVal() == DOT){
                play(row-1, col);
                breakLoop = true;
            }
            else if (turn == 1 && hexCells[(row-1)*size + col].getVal() == DOT) {
                play(row-1, col);
                breakLoop = true;
            }
...
/*
    ...
    return const string.
*/
const string HexArray1D::GetMove(bool& isCommand) const {
...
void HexArray1D::ParseCommand(const std::string& command) {
...
*/
void HexArray1D::writeToFile(ofstream& outfile) const{
    ...
    for (int i=0; i < size; ++i)
        for (int j=0; j < size; ++j)
            outfile << hexCells[i*size + j].getVal();
    outfile << endl;
...
*/
void HexArray1D::readFromFile(ifstream& infile) {
    ...
    for (int i=0; i < size; ++i)
        for (int j=0; j < size; ++j)
            hexCells[i*size + j].setVal(dummy.at(j + i * size) - '0');
...
/*
    ...
    return FinalMove;
*/
FinalMove HexArray1D::SimulateMoves(){
...
/*
    ...
    return bool
*/
bool HexArray1D::IsValidAI(int row, int col) const {
    if (row >= 0 && row < size && col >= 0 && col < size && hexCells[row*size + col].getVal() == DOT)
        return true;
...
*/
void HexArray1D::ResetVisiteds() {
    for (int i = 0; i < size; ++i)
        for (int j = 0; j < size; ++j)
            hexCells[i*size + j].setVisited(false);    
}
...
/*
    ...
    return bool
*/
bool HexArray1D::CheckP1(int row, int col, bool found) {
    
    if (!IsValidP1(row, col))
        return false;
    
    if (row == size-1 && hexCells[row*size + col].getVal() == O) {
        hexCells[row*size + col].setVisited(true);
        return true;
    }
...
/*
    ...
    return bool
*/
bool HexArray1D::CheckP2(int row, int col, bool found) {

    if (!IsValidP2(row, col))
        return false;
    
    if (col == size-1 && hexCells[row*size + col].getVal() == X) {
        hexCells[row*size + col].setVisited(true);
        return true;
    }
...
/*
    ...
    return bool
*/
bool HexArray1D::IsValidP1(const int& row, const int& col) {

    if (row >= 0 && row < size && col >= 0 && col < size && 
        hexCells[row*size + col].getVal() == O && !hexCells[row*size + col].getVisited()){

        hexCells[row*size + col].setVisited(true);
        return true;
    }
    else if (row >= 0 && row < size && col >= 0 &&
            col < size && hexCells[row*size + col].getVal() == DOT) {
...
/*
    ...
    return bool
*/
bool HexArray1D::IsValidP2(const int& row, const int& col) {

    if (row >= 0 && row < size && col >= 0 && col < size && 
        hexCells[row*size + col].getVal() == X && !hexCells[row*size + col].getVisited()){
...
bool HexArray1D::isEnd() {
    ...
    if (turn == 0) {    // Which means check if player1 finished the game         
        ...
        while (row < size && !finished) {
            ...
            if(hexCells[row*size + col].getVal() == X)    
...
int HexArray1D::operator()(int row, int col) const throw (MyException) {

    if (row >= size || col >= size){
        throw MyException();
    }
    return hexCells[row*size + col].getVal();
}

bool HexArray1D::operator==(const AbstractHex& AH) {
...
HexArray1D& HexArray1D::operator=(AbstractHex* AbsHex){

    if(this == AbsHex){
        return *this;
    }

    if(this->size != AbsHex->getSize()){
        delete hexCells;
        hexCells = nullptr;
        this->size = AbsHex->getSize();
        createCells();
    }
    this->end = false;
    this->lastmove = AbsHex->lastMove();
    this->numOfMoves = 0;
    this->opponent = AbsHex->getOpponent();
    this->turn = 0;
    this->posList = AbsHex->getAI_POS_List();
